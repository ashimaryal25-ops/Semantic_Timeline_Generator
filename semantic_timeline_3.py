...
def main():
    ...
    if event_list:
        ...
        # Partition the data by Cluster ID
        ...
        st.subheader("Check any topics to see all the related events")
    ...
    st.subheader("Timeline of all events:")
...
def create_timeline(event_list):
    ...
    return all_items # Return the full list
...
def display_timeline(data_items):   
    ...
    for item in data_items: 
        ...
        # Access start_date from the individual item
    ...
    # pass the dictionary containing the list of events to the timeline component
...
def cluster(event_list, model):
    ...
    # 2. Normalize for Cosine Similarityproxy
    emb_norm = normalize(embeddings).astype('float64')

    # 3.  pass parameters to the Class
...
@st.cache_data
def generate_auto_topic(grouped_events, _summarizer):
    combined_text = " ".join(e["Sentence"] for e in grouped_events)

    result = _summarizer(combined_text, max_length=10, min_length=3, do_sample=False)
